...
function decrypt_packet(key, address, packet)
{
    // Build nonce
    // [0] - [2] = reversedAddress[0] - reversedAddress[2]
    // [3] - [7] = packet[0] - packet[4]
    var a = Buffer.from(address.replace(/:/g,""), "hex");
    a = reverseBuffer(a);
    var nonce = Buffer.alloc(3 + 5);
    for (var i = 0; i < 3; i++)
    {
    ...
    for (var i = 0; i < 5; i++)
    {
        nonce.writeUInt8( packet[i], 3 + i );
    }

    // Decrypt Payload
    var payload = crypt_payload (key, nonce, packet.slice(7));

    // Compute checksum
    var check = make_checksum (key, nonce, payload);

    // Check bytes
    console.log("Decrypt check bytes");
    console.log( check.slice(0,2) );
    console.log( packet.slice(5,7) );
    if ( check.slice(0,2) != packet.slice(5,7) )
    {
        return 0;
    }

    // Decrypted packet
    var dec_packet = Buffer.concat( [packet.slice(0,7), payload] );
    return dec_packet;
};
...
module.exports.decrypt_packet = decrypt_packet;
